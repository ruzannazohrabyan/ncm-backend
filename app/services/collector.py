...
def ssh_pull_direct(
    ...
    username: str,
    password: str,
) -> tuple[str, str, DeviceFacts]:
    """
    Connect to a device via SSH, auto-detect the OS if unknown, pull the
    running configuration, and return ``(config_raw, detected_os_type, facts)``.
    ...
    returned — the config pull itself is the primary contract of this
    function and must not be aborted by ancillary failures.
    ...
    logger.info(
        ...
        f"{len(output)} chars | os={os_type!r} | facts={facts.as_dict()}"
    )
    return output, os_type, facts
